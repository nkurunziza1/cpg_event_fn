...
const Slide = ({
  ...
  const { image, button, title, description, _id } = slide;
    ...
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      ...
      >
        ...
        >
          ...
          <p className="text-start py-4" dangerouslySetInnerHTML={{ __html: shortDescription || "" }} />
          <div className="flex justify-center">
            <button
              onClick={() => handleReadMore(_id as string)}
...
const CarouselControl = ({
  ...
  return (
    <button
      className={`w-10 h-10 flex items-center mx-2 justify-center bg-primary dark:bg-neutral-800 border-3 border-transparent rounded-full focus:border-[#6D64F7] focus:outline-none hover:-translate-y-0.5 active:translate-y-0.5 transition duration-200 ${type === "previous" ? "rotate-180" : ""
        }`}
      title={title}
      onClick={handleClick}
    >
        ...
        >
          ...
          >
            {slides.map((slide, index) => (
              <Slide
                key={slide._id || index}
                handleReadMore={() => handleReadMore(slide?._id as string)}
                slide={slide}
                index={index}
                current={current}
